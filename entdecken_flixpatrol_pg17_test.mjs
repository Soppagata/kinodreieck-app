...
try {
  ...
  sql(`
    ...
    create table public.kd_flixpatrol_vocabulary_cache (resource_type text, source_id text, name text, code text);
    insert into public.kd_flixpatrol_vocabulary_cache values
      ('countries','cnt_gGE4RaeXpyz2U9Q5tEMYDwri','Austria','AT'),
      ('companies','cmp_qypvowjqFhEIpCc0HlQ6VoYk','Amazon Prime',null),
      ('companies','cmp_oGtsgdpOrjIu3XzTEnWPt87Y','Disney+',null),
      ('companies','cmp_VvmYc7OphiUds0Hgjbz5MESn','Apple TV',null),
      ('companies','cmp_phDSns8OP1rtHnX6QwlEKhiq','Apple TV Store',null);
  ...
  check("Fehlerrow behält den alten Format-6-Payload bis zum vollständigen Save", () => {
    ...
    assert.equal(readback.feed.format, 8);
  });
  check("Authenticated kann die service-role-only Verträge nicht direkt ausführen", () => {
    const result = spawnSync(join(selected.dir, "psql"), psqlArgs, {
      input: `begin; set local role authenticated; select set_config('request.jwt.claim.role','authenticated',true); select public.kd_entdecken_public_feed_readback(8);`,
      encoding: "utf8", env,
    });
