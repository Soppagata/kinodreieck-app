...
function validateMixedRecord(value, feed, errors, index) {
  ...
  if (value.sourceId === "chart:joyn-at") {
    const expectedPrefix = value.mediaType === "film"
      ? "/filme/" : value.mediaType === "series" ? "/serien/" : null;
    if (value.sourceLabel !== "Joyn Österreich"
        || !expectedPrefix || url?.hostname !== "www.joyn.at"
        || !url.pathname.startsWith(expectedPrefix) || url.pathname.length <= expectedPrefix.length
        || url.search || value.availability?.market !== "streaming"
        || value.availability?.service !== "Joyn" || !licenses?.length
        || value.popularity?.metric !== "source-chart-rank" || value.popularity?.value !== null
        || value.popularity?.measuredOn !== feed.refreshedOn) {
