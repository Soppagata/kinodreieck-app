...
-- GitHub-Zeitplaene duerfen verspaetet starten. Die Faelligkeit wird deshalb
-- nur ueber den bereits atomar gespeicherten Wiener Kalendertag entschieden;
-- Uhrzeit, Lease und ein verbrauchter Tagesversuch bleiben getrennte Fakten.
...
begin
  ...
  v_last_attempt_day := coalesce(
    v_feed.last_attempt_on,
    (v_anchor at time zone 'Europe/Vienna')::date
  );
  ...
  if not coalesce(v_enabled,false) then v_status := 'disabled';
    ...
    v_status := case
      when v_feed.status = 'ready'
       and (v_feed.last_success_at at time zone 'Europe/Vienna')::date = v_today
        then 'not_due'
      when v_feed.status = 'error' then 'failed'
      else 'held'
    end;
  else
...
comment on function public.kd_entdecken_weekly_refresh_claim(text) is
  'Scheduled: einmal je Europe/Vienna-Kalendertag, auch bei verspaetetem GitHub-Start. Owner: nur mit Staging-Override. Lease/Fence und maxAttempts 1 verhindern Parallelitaet und automatische Retries.';
