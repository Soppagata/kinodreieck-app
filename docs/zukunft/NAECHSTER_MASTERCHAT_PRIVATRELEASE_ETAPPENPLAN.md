...
P1-Autorisierungsblocker: Der Ausführungswächter hat die lokale Änderung an
`kontoSicherAutomatischLaden` (Kontostand trotz abweichendem getrennten
Gastbestand anzeigen) sowie die Änderung an `src/services/catalog.js`
(öffentliche Demo-Fallbacks durch ausschließlich berechtigten Kontozugriff
ersetzen) sowie die Localmodus-Navigation in `App.jsx` (bereitgestellte
Kino-/Streaming-/Entdeckenwege nur für Konten) abgelehnt. Alle drei Schritte
bleiben unangewandt; keine alternative
oder indirekte Umgehung. Dafür wird eine ausdrücklich datei-/wirkungskonkrete
Bestätigung von Max benötigt. Der unabhängige Minimal-Login-/Legal-Teil und
E4 sind als lokale Commits integriert. Neue oder noch ungebundene Konten
bleiben ehrlich im Gate, statt Kontodaten oder einen fertigen Loginweg
vorzutäuschen. Der P1-Checkpoint ist weder vollständiger PR-01/02-Abschluss
noch ein lieferbarer Privatrelease.

Integrationsreihenfolge angepasst am P1-Blocker: Planstand → unabhängiger
P5/E4-Commit → kohärenter P1-Checkpoint → notwendige kleine Testnaht → einmal
FINAL_LOCAL des begrenzten lokalen Kandidaten. Kein vollständiger E1- oder
Releaseabschluss, solange die drei Freigaben fehlen. Die weiteren Pakete werden erst aus dem
integrierten Stand zugeschnitten; es gibt keinen zweiten Integrationsowner.
