...
### Aktueller Meisterstand und Welle W2 – 02.09.2026

Gemeinsamer, noch nicht veröffentlichter Integrationskandidat ist
`be77b0b` auf `codex/road-to-live-integration-20260902`. Er führt den auf
`staging` und `main` ausgelieferten Login-/Konto-Recovery-Stand `24bbddc` mit
den vorbereiteten E1-, E2- und E3a-Grundlagen sowie der integrierten W2
...
| M3 | Feedback ohne Namensangabe und authentifizierte Löschanfrage laufen über einen begrenzten, servergebundenen Mailweg | GEBAUT | Endpoint `484a63b` und Browseroberfläche `a300756` sind lokal integriert und standardmäßig fail-closed. Die freigegebene Resend-Vertragsprobe ist mit exakt `3/3` Requests belegt (`200` neu, `200` idempotent wiederholt, `409` bei verändertem Body); kein Retry. Adapter, Migration, Aktivierung und Praxis sind offen |
| M4 | Nur tatsächlich aktive automatische KI-Jobs erhalten genau eine Nachprüfung nach sechs Stunden und höchstens einen Retry | OFFEN | GitHub-Workflow ist aktiv, aber die letzten drei planmäßigen Läufe scheiterten; Lauf `33599267721` vom 02.09.2026 endete für Radar und Entdecken bereits am Endpoint mit HTTP 403. Damit ist kein tatsächlich laufender automatischer Anbieterjob belegt; E3b folgt erst nach einem funktionierenden Initialtrigger und gebundenem Mailtransport |
| M5 | Bereinigter, integrierter Privatrelease-Kandidat mit einem gemeinsamen Abschlusslauf | OFFEN | W2 besitzt auf `be77b0b` einen grünen gemeinsamen lokalen Zwischenabschluss; der vollständige RC folgt erst nach M4, E5 und den gebundenen Außenwirkungen |
...
| PR-10 / P6, E6 | Ein exakter integrierter RC: vollständige Mocksuite/Production-Build/finaler Diff-/Secret-/Bundlecheck; anonyme Daten-/Demo-/Backup-/Mapgrenze; relevante RLS/Kontotrennung/Readbacks, Backup/Restore nur an nötiger Mutation; eine Praxismatrix Login/Legal/leer lokal/Eintrag+Refresh/Login ohne Merge/Gast nach Logout/A-B-Grenze/Sync+zweites Gerät+Konflikt+defekter Cache/Sicherheitsdownload+Bestätigung/Export oder manuelle Rechte/Löschmail/Feedback/Kinomobil/Radar+Entdecken/KI-aus+Anbieterausfall/aktiver 6h-Checker/PWA+Safari/iPhone+Tastatur+320/393/Desktop; bestehende Belege übernehmen; schmaler Betrieb für Sync/Functions/Mail/Kosten/Support/Cache/Rollback; keine neue Featurelogik in E6; RC/Push/CI/Backend/Deploy/Praxis/Release-Tag getrennt; private Lieferung kinodreieck.at nur autorisiert, keine Ankündigung/Demo/Registrierung, höchstens 15 Einladungen | Meister / Integrationsowner | BEGRENZTES `FINAL_LOCAL` bis einschließlich W2 auf `be77b0b` GRÜN, noch kein vollständiger RC: Standard-`npm test` Exit 0 mit allen Mock-Suiten sowie Single-File-, Vite-Production- und Pages-Build; die früher zeitabhängige Radarprobe ist in diesem unveränderten Lauf ebenfalls grün. W2-fokussiert: Account-Größenweg 12/12 plus PostgreSQL-ACL f/f/f/t, Mail-Endpoint 15/15, Mail-UI 18/18, Mail-Vertrag 70/70, Ledger 15/15 und Resend-Vertrag 9/9. Finaler Diffcheck leer; geprüfte Secretmarker, Source Maps und öffentlicher Demo-Sidecar fehlen im Webpaket. Die anschließend freigegebene Resend-Vertragsprobe verbrauchte exakt `3/3` synthetische Requests, ohne Retry, DNS, Nutzerdaten oder Produktversand. Praktischer Browserdownload, physisches zweites Gerät, reale Max-Datenmessung, Mail-Migration/Provider/Aktivierung/Praxis, E3b/E5, vollständiger RC, OSS-Beleg sowie Push/CI/Deploy bleiben OFFEN |
