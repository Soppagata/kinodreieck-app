/* Dauerhafter, inhaltsfreier Beleg fuer genau eine empfangene Providerantwort.
   Die Adapter liefern ausschliesslich bereits normalisierte Felder hinein. Der
   Rohtext wird nur im Speicher gehasht und weder im Receipt noch anderweitig
   von diesem Modul zurueckgegeben. */
...
const MAX_RAW_RESPONSE_BYTES = 8 * 1024 * 1024;
...
export async function createProviderReceipt({
  ...
  if (!PROVIDERS.has(provider)
      ...
      || new TextEncoder().encode(rawResponse).length > MAX_RAW_RESPONSE_BYTES
  ...
  return Object.freeze({
    ...
    model,
    usage,
    responseSha256: await sha256Hex(rawResponse),
    resultMode,
    server,
  });
}
