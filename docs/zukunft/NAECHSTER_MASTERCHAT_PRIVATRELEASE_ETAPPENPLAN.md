...
### Aktueller Meisterstand und E3a-Checkpoint – 02.09.2026

Gemeinsamer, noch nicht veröffentlichter Integrationskandidat ist
`4731c0c` auf `codex/road-to-live-integration-20260902`. Er führt den auf
...
Resend-Adapter, Legal-Abschluss und Release-Wiring
zusammen. Der Primärcheckout
...
| M3 | Feedback ohne Namensangabe und authentifizierte Löschanfrage laufen über einen begrenzten, servergebundenen Mailweg | GEBAUT, AUSSENWIRKUNG TEILBELEGT | Integrierter HEAD `4731c0c`; vollständiges `npm test` grün. Beide E3a-Migrationen sind remote angewandt und per Ledger-/Objekt-/Rechte-Readback grün. Die vier Basissecrets sind gesetzt, das separate Aktivierungssecret fehlt weiterhin. `private-mail-request` ist remote `ACTIVE`, Version 1, `verify_jwt=true` und ohne Aktivierung fail-closed. Der Kandidat bindet den Transport serverseitig ausschließlich an Staging und der Deployment-Smoke erwartet den privaten anonymen Rechtestopp. Die Resend-Domains-API antwortete mit `401 restricted_api_key`; es gab keine Domainmutation und keinen Provider-Send. Staging wurde nicht gepusht. Blockiert bleiben Resend-Domain-Onboarding, DNS-Authentifizierung und Resend-Dashboard-Login |
| M4 | Nur tatsächlich aktive automatische KI-Jobs erhalten genau eine Nachprüfung nach sechs Stunden und höchstens einen Retry | OFFEN | GitHub-Workflow ist aktiv, aber die letzten drei planmäßigen Läufe scheiterten; Lauf `33599267721` vom 02.09.2026 endete für Radar und Entdecken bereits am Endpoint mit HTTP 403. Damit ist kein tatsächlich laufender automatischer Anbieterjob belegt. E3b ist die nächste Etappe, beginnt aber erst nach erfolgreichem E3a-Staging-Push; davor keine Job-/Retrylogik an einen unbewiesenen Mail-/Initialtrigger anbauen |
| M5 | Bereinigter, integrierter Privatrelease-Kandidat mit einem gemeinsamen Abschlusslauf | OFFEN | Der integrierte E3a-Stand `4731c0c` hat ein grünes vollständiges `npm test`; der vollständige RC folgt erst nach erfolgreichem E3a-Staging-Push, M4, E5 und den übrigen gebundenen Außenwirkungen |
...
E3a-Checkpoint auf `4731c0c`: Die vollständige lokale `npm test`-Suite ist
...
`private-mail-request` ist `ACTIVE` in Version 1 mit `verify_jwt=true` und
...
damit gab es keinen Claim oder Transport. Der Versuch, die Domainlage über die
Resend-Domains-API zu lesen, endete ohne Mutation mit
`401 restricted_api_key`. Es gab keinen Provider-Send und keinen
Staging-Push. Offene Wirkungsblocker sind Resend-Domain-Onboarding, DNS-
Authentifizierung und Resend-Dashboard-Login. Erst ein erfolgreicher
E3a-Staging-Push öffnet die nächste Etappe E3b.
...
| PR-05 / P2, E2 | Einmal vollständigen aktuellen Max-Kontoumfang der BEHALTEN-Flächen messen; regenerierbare Kataloge/Suchergebnisse/Feeds/Bilder/Caches nicht pro Konto duplizieren; höchstens 15 Einladungen organisatorisch; keine 250-MB-Hardcap, Kapazitätsrechnung, Kompression, automatische Löschung, neues Archiv oder 400-MB-Monitoring | Baumeister Kontodaten; reale Messung später Meister | GEBAUT als Messweg bis `2fd5674`: service-role-only RPC und Einmal-CLI decken alle 13 Releaseklassen ab und geben ausschließlich Klasse, Zeilen und Bytes sowie die Gesamtsumme aus; gemeinsame regenerierbare Katalog-/Feed-/Bild-/Cache-/Programmdaten bleiben ausgeschlossen. Migration und echte Abfrage wurden nicht ausgeführt. Die einmalige aggregierte Vermessung des vollständigen aktuellen Max-Kontos bleibt OFFEN; keine Inhaltslogs, keine 250-MB-Grenze und keine Kapazitätszusage |
| PR-06 Daten/Recht / P3, E3a | Tatsächliche Browser-/Server-/Sync-/Revisions-/Sicherheitsdownload-/SW-/Sitzungs-/Geräte-/KI-Präferenz-/Diagnose-/Supportdaten verständlich zuordnen; pro aktiver KI-/Suchfunktion konkrete Felder/Empfänger, Infrastruktur getrennt; persönliche KI serverseitig berechtigt/fail-closed; reale Aufbewahrung/Exportumfang/manueller Rechteweg; Löschanfrage authentifiziert und manuell bearbeitet, kein Autoarchiv/Self-Delete; keine ungefragte Diagnose; ein Legal-Zugang vor Login, nötiger neutraler Kontakt nur dort, privater Empfänger serverseitig; Analytics aus, nur notwendiger Speicher, kein Banner; Endtexte rechtlich freigeben | Meister bis Dispatch Datenschutz/Mail | GEBAUT und auf `4731c0c` integriert: tatsächliche Browser-/Sync-/Revision-/SW-/Sitzungs-/KI-/Diagnose-/Supportwege, manueller Rechteweg und Resend-Hinweis zu US-Verarbeitung/30 Tagen sind in einer kombinierten Legal-Fläche zugeordnet; Max' neutraler Kontakt ist nur dort sichtbar, der private Empfänger bleibt serverseitig. Vollständiges `npm test` grün. Formelle rechtliche Endfreigabe, Domain/DNS, Staging-Push und Praxis bleiben OFFEN |
| PR-06 Nachprüfung / P4, E3b | Nur tatsächlich aktive automatische Anbieterfunktionen; initial dauerhaft Job-ID/Funktion/initiale Operation/triggered_at/check_due_at=+6h; atomarer einmaliger fälliger Claim; exakten Initialvorgang prüfen, Reservierung nicht Erfolg; Erfolg nur fertig + finalisierte positive Kosten + gültige Usage/Token; sonst genau ein normaler Retry mit allen Not-Aus-/Rechte-/Provider-/Budget-/Kosten-/Idempotenzgates; gleiche Job-ID/neue Operation/retry_consumed=true; kein neuer Termin/Checkerjob/dritter Versuch; danach eine Betriebs-Mail; keine Jobs für manuelle Aktionen, Mocks, Tests, inaktive Scheduler oder providerfreies Entdecken | Meister bis Dispatch KI-Nachprüfung | OFFEN; der interne Betriebs-Mail-Inhaltsvertrag ist vorbereitet. Readback 02.09.2026: Workflow aktiv, aber alle drei bisherigen planmäßigen Läufe fehlgeschlagen; Lauf `33599267721` endete für Radar und Entdecken bereits mit HTTP 403. Kein tatsächlich laufender automatischer Anbieterjob ist belegt. E3b startet ausdrücklich erst nach erfolgreichem E3a-Staging-Push; funktionierender Initialtrigger, gebundener Transport und E6-Abnahme bleiben offen |
| PR-07 Feedback/Löschanfrage / P3, E3a | Feedback ohne Namensangabe, kein mailto; fester Domainabsender/privater Serverempfänger/fester Betreff; keine freien Header/Adressen/Anhänge/fremdes HTML; escaped Text + HTML; keine App-DB-Ablage/Konto-/Profil-/Diagnose-/Browseranreicherung oder Sitzungsverknüpfung im Inhalt/Produktlog; Text-/Rate-/Originlimits, kein Relay; ehrliche Erfolgs-/Fehleranzeige ohne Rohfehler, Text erhalten/kopierbar; technische Zustellmetadaten erklären; Domain-Onboarding/Absenderauth/Transportverfügbarkeit fokussiert; separater authentifizierter Löschanfragetyp mit notwendiger Kontoidentität und manueller Folgeaktion | Meister bis Dispatch Datenschutz/Mail | GEBAUT und auf `4731c0c` integriert; vollständiges `npm test` grün. Beide E3a-Migrationen sind remote samt Readback grün. Vier Basissecrets sind gesetzt, das Aktivierungssecret fehlt. `private-mail-request` ist `ACTIVE` v1 mit `verify_jwt=true` und bleibt fail-closed; der Kandidat akzeptiert serverseitig nur Staging. Die Resend-Vertragsprobe `200/200/409` bleibt der einzige Providerbeleg; die Domains-API endete mit `401 restricted_api_key` ohne Mutation. Kein Provider-Send und kein Staging-Push. Domain-Onboarding, DNS-Authentifizierung und Resend-Dashboard-Login bleiben BLOCKIERT |
