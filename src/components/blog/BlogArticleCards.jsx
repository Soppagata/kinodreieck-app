...
export function BlogArticleCards({ cards = [], scope, actions, onNotice }) {
  ...
  return <div className="kd-blog-card-grid">{cards.map((card) => <article className="kd-blog-card" key={card.articleId}>
    ...
    {card.referencePreview?.length ? <BlogReferenceList references={card.referencePreview}
      ordered={card.ordered === true || card.referencePreview.some((reference) => Number.isInteger(reference.rank))}
      previewLimit={3} editable={false} actions={actions} /> : null}
    {card.publicationError ? <p role="status" className="kd-blog-error">Veröffentlichung: {card.publicationError === "unknown" ? "Ergebnis wird geprüft." : "Aktion fehlgeschlagen."}</p> : null}
    <div className="kd-blog-card-actions"><button type="button" className="kd-blog-button kd-blog-button-primary" onClick={() => read(card)}>Lesen</button>
      {scope === "private" ? <><button type="button" className="kd-blog-button kd-blog-button-quiet" onClick={() => actions.onEditArticle({ articleId: card.articleId })}>Bearbeiten</button>
        <details className="kd-blog-card-menu"><summary aria-label={`Weitere Aktionen für ${card.title}`}>⋯</summary><div className="kd-blog-menu-panel">
          ...
          {card.publicationError?.operationId ? <button type="button" onClick={() => void actions.onRetryPublication({ articleId: card.articleId, operationId: card.publicationError.operationId })}>Veröffentlichung prüfen</button> : null}
