...
for (const width of [320, 393, 1280]) {
  test(`PR-08: Kino-Filter bei ${width}px lokal bedienbar`, async ({ page }) => {
    ...
    await page.getByLabel('Datum im Kinoprogramm').selectOption('31.8.');
    await page.getByLabel('Kino im Kinoprogramm').selectOption('Gartenbaukino');
    await page.getByLabel('Genre im Kinoprogramm').selectOption('action');
    ...
    const terminmix = page.locator('[data-kino-suchtreffer="programm:1005"]');
    await terminmix.locator('.kd-kompakt-eintrag > div').first().click();
    await expect(terminmix).toContainText('22:00');
    await expect(terminmix).not.toContainText('12:00');
    await expect(terminmix).not.toContainText('20:00');
    const reset = page.getByRole('button', { name: 'Filter zurücksetzen', exact: true });
    await reset.click();
    await expect(reset).toHaveCount(0);
    await page.getByLabel('Rest ab', { exact: true }).fill('19:00');
    await page.getByRole('button', { name: 'Zeitfilter an', exact: true }).click();
    await expect(page.locator('[data-kino-suchtreffer^="programm:"]')).toHaveCount(3);
    await expect(page.locator('[data-kino-suchtreffer="film:9"]')).toHaveCount(0);
    await expect(page.locator('[data-kino-suchtreffer="programm:1005"]')).toContainText('20:00');
    await expect(page.locator('[data-kino-suchtreffer="programm:1005"]')).toContainText('22:00');
    await expect(page.locator('[data-kino-suchtreffer="programm:1005"]')).not.toContainText('12:00');
    await filter.click();
    await expect(filter).toContainText('Filter · 1');
    await expect(page.locator('.kd-kino-filterhinweis')).toHaveText('Kinoprogramm: ab 19:00');
    await reset.click();
    ...
    await reset.click();
    await expect(page.locator('[data-kino-suchtreffer^="programm:"]')).toHaveCount(5);
