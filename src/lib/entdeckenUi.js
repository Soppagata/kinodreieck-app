...
export function createRadarCatalogIndex({
  ...
  const rows = [
    ...
    ...[...list(streamingKnown?.titel), ...list(streamingDiscover?.titel)].map((entry) => ({
  ...
  return Object.freeze([...byTargetId.values()].map((entry) => {
    const aliases = uniqueText(entry.aliases);
    const years = [...entry.years];
    const year = years.length === 1 ? years[0] : null;
    const baseKeys = uniqueText(aliases.map(norm)).filter(Boolean);
    const searchKeys = uniqueText([
      ...baseKeys,
...
export function searchRadarCatalog(index, query) {
  ...
  const buckets = [[], [], [], []];
  for (const entry of list(index)) {
    const keys = list(entry?.searchKeys);
    let rank = -1;
