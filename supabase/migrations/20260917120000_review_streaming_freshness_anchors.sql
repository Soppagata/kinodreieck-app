-- Preserve source expiry and qualified personal window anchors in progressive responses.
-- Additive replacement; existing request fields, RLS and cursor rules stay intact.
begin;

create or replace function public.kd_streaming_page_new_state(p_payload jsonb,p_services text[],p_stands jsonb,
  p_comparisons jsonb,p_now timestamptz,p_anchor_start timestamptz,p_consumed_until timestamptz,
  p_has_anchor boolean,p_legacy_start timestamptz) returns jsonb
...
declare initial_services text[]:=public.kd_streaming_page_services(p_payload); current_services text[]; selected text;
  ...
  motn_at timestamptz; anchor_at timestamptz; diffs jsonb; motn_entries jsonb; duration_seconds constant bigint:=1209600;
begin
  ...
  if begin_at is null and p_legacy_start<=p_now
    and extract(epoch from p_now)<extract(epoch from p_legacy_start)+duration_seconds then begin_at:=p_legacy_start; consumed_until:=p_legacy_start; end if;
  anchor_at:=begin_at;
  ...
  return jsonb_build_object(
    'newSince',case when begin_at is not null
      and extract(epoch from p_now)<extract(epoch from begin_at)+duration_seconds then begin_at end,
    'fensterBeginn',extract(epoch from anchor_at)*1000,
    'verbrauchtBis',extract(epoch from consumed_until)*1000);
end
$$;

-- The scalar helper stays compatible with existing readers and tests.
...
language sql stable set search_path = pg_catalog, public as $$
  select public.kd_streaming_page_timestamp(public.kd_streaming_page_new_state(
    p_payload,p_services,p_stands,p_comparisons,p_now,p_anchor_start,p_consumed_until,p_has_anchor,p_legacy_start)->>'newSince')
$$;
revoke all on function public.kd_streaming_page_new_state(jsonb,text[],jsonb,jsonb,timestamptz,timestamptz,timestamptz,boolean,timestamptz) from public, anon, authenticated;
...
declare v_role text:=auth.role(); v_account uuid:=auth.uid(); v_services text[]; v_view text;
  ...
  v_expiry timestamptz; v_anchors jsonb; v_next text; v_now timestamptz:=statement_timestamp();
begin
  ...
  with selected as materialized (
    ...
    select e.output_key,public.kd_streaming_page_new_state(
        ...
        e.anchor_start,e.consumed,e.has_anchor,e.legacy_start) new_state
    ...
    select e.output_key,e.library_id,e.seen,e.must_watch,e.rated,
      public.kd_streaming_page_timestamp(n.new_state->>'newSince') new_since,n.new_state
      from personalized e left join new_labels n using(output_key)
  ), filtered as materialized (
    select s.output_key,l.library_id,l.new_since,l.new_state,s.title_order,s.provider_sort,s.work_type,s.release_year
    ...
    select output_key,library_id,new_since,new_state,row_number() over(order by
  ...
  select jsonb_build_object('all',count(*)::integer,
    ...
    coalesce((select jsonb_agg(jsonb_build_object('id',payload->>'watchmode_id',
      'fensterBeginn',new_state->'fensterBeginn','verbrauchtBis',new_state->'verbrauchtBis') order by page_order)
      from page_payloads where payload->>'watchmode_id' ~ '^[1-9][0-9]*$'
        and new_state->>'fensterBeginn' is not null and new_state->>'verbrauchtBis' is not null),'[]'::jsonb)
    into v_counts,v_total,v_items,v_expiry,v_anchors from labels;
  ...
  return jsonb_build_object('format',1,'status','ready','region','AT','version',v_version,
    ...
    'complete',v_next is null,'nextExpiryAt',v_expiry,
    'sourceExpiresAt',public.kd_streaming_page_timestamp(v_meta->>'gueltig_bis'),
    'newAnchors',v_anchors,'meta',v_meta||jsonb_build_object('motn_checked_at',
