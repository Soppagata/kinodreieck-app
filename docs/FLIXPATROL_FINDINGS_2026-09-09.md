...
| F16 / 3 | Zusaetzlicher Trendkontext fuer Entdecken: `src/lib/popularityContracts.js`, `src/lib/entdeckenUi.js` | `hoursviewed`, `rankingsofficial` und `mostwatched` koennen spaeter Netflix-Wochen-/Berichtsmetriken liefern; Wikipedia-/Personen- und Traileraufrufe allenfalls datierte Aufmerksamkeitssignale. [API-Uebersicht](https://flixpatrol.com/api2/), [Hours Viewed](https://flixpatrol.com/api2/endpoint-hoursviewed/) | Aktuell keine zusaetzlichen Requests empfohlen: Die offizielle Netflixquelle liefert bereits den benoetigten Rang. Metriken nur mit Einheit und Zeitraum speichern; keine Doppelhaltung desselben Wochenrangs, keine Umrechnung von Aufrufen in persoenliche Passung oder Filmqualitaet. |
| F17 / 1 | Gemeinsames Quellenvokabular und Zeitbezug im kuenftigen Adapter | Companies/Countries/Genres/Keywords/Regions/Languages und gegebenenfalls `time` helfen, API-IDs, Begriffe und Zeitfelder einmal korrekt abzubilden. [Referenzdaten und Time](https://flixpatrol.com/api2/) | Kleine gemeinsame Zuordnungstabellen statt Listenabruf pro Titel oder Verbraucher. Apple-TV-Dienst und Apple-Store getrennt halten, Oesterreich ueber die bestaetigte Laender-ID binden. Abrufzeit in UTC und unveraenderten Anbieter-Zeitwert getrennt halten; Zeitzone nicht erfinden. |

Weitere angebotene Metriken wie geschaetzte Dienst-Abonnentenzahlen oder
Torrent-Popularitaet haben fuer die jetzigen Nutzerwege keinen belegten Nutzen.
Die als deprecated markierten Datenbank-Popularitaets-/Social-Fans-Endpunkte
werden ebenso wenig als neue Basis vorgeschlagen. Das verhindert zusaetzliche
Verbraucher ohne konkreten Zweck; es ist keine kuenstliche Kontingentsperre.
