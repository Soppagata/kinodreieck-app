...
`78f1149` auf `codex/road-to-live-integration-20260902`. Er führt den auf
...
| M3 | Feedback ohne Namensangabe und authentifizierte Löschanfrage laufen über einen begrenzten, servergebundenen Mailweg | GEBAUT, PROVIDERWEG BELEGT, STAGING-PUSH OFFEN | Kandidat `78f1149`: Der redundante Browser-Requestheader ist entfernt und der Clienttimeout lässt dem 20-Sekunden-Serverweg Abschlussluft; fokussierte UI-/Endpoint-/Adapter-/Postgres-Prüfungen sind grün. Beide E3a-Migrationen sind remote angewandt. Die vier Basissecrets sind digestgenau rückgelesen, das Aktivierungssecret ist separat gesetzt, und `private-mail-request` ist remote `ACTIVE`, Version 2, `verify_jwt=true` sowie über fünf Dateien bytegleich zum lokalen Serverstand. Die vier Resend-DNS-Einträge sind in Cloudflare gesetzt, beide Pages-CNAMEs blieben erhalten, öffentliche DNS-Sicht und Resend-Domainstatus sind verifiziert. Genau ein synthetischer Staging-Request endete ohne Retry mit HTTP 200 `accepted`; der inhaltsfreie Ledger zeigt eine akzeptierte Operation und zwei Rate-Buckets, Resend meldet `delivered`. Staging wurde noch nicht gepusht |
...
| M5 | Bereinigter, integrierter Privatrelease-Kandidat mit einem gemeinsamen Abschlusslauf | OFFEN | Der integrierte E3a-Stand `78f1149` baut auf einem grünen vollständigen `npm test` auf; die fokussierte PWA-Mailkorrektur ist zusätzlich grün. Der vollständige RC folgt erst nach erfolgreichem E3a-Staging-Push, M4, E5 und den übrigen gebundenen Außenwirkungen |
...
E3a-Checkpoint auf `78f1149`: Die vollständige lokale `npm test`-Suite auf
dem vorherigen E3a-Codeabschluss ist grün. Die anschließende fokussierte
...
UI-, Endpoint-, Adapter- und Postgres-Guardprüfungen sind danach grün.
...
Resend-Status `delivered`. Es gab keinen zweiten Send. Staging-Push, CI,
Custom-Domain-/Service-Worker-Readback und physische iPhone-Abnahme bleiben
noch offen.
...
Staging-Push. Der DNS-/Providerblocker ist mit dem oben dokumentierten
Einmallauf geschlossen. Erst der weiterhin offene erfolgreiche
...
| PR-07 Feedback/Löschanfrage / P3, E3a | Feedback ohne Namensangabe, kein mailto; fester Domainabsender/privater Serverempfänger/fester Betreff; keine freien Header/Adressen/Anhänge/fremdes HTML; escaped Text + HTML; keine App-DB-Ablage/Konto-/Profil-/Diagnose-/Browseranreicherung oder Sitzungsverknüpfung im Inhalt/Produktlog; Text-/Rate-/Originlimits, kein Relay; ehrliche Erfolgs-/Fehleranzeige ohne Rohfehler, Text erhalten/kopierbar; technische Zustellmetadaten erklären; Domain-Onboarding/Absenderauth/Transportverfügbarkeit fokussiert; separater authentifizierter Löschanfragetyp mit notwendiger Kontoidentität und manueller Folgeaktion | Meister bis Dispatch Datenschutz/Mail | GEBAUT auf `78f1149`; fokussierte Abschlussprüfungen grün. Migrationen, digestgebundene Secrets, bytegleiche `ACTIVE`-Function v2/JWT und Staging-CORS sind remote belegt. Cloudflare-DNS und Resend-Domain sind verifiziert. Genau ein synthetischer Staging-Send endete ohne Retry mit HTTP 200 `accepted`, passendem inhaltsfreien Ledger und Resend `delivered`. Staging-Push, Deploy-/PWA-Readback und physische iPhone-Praxis bleiben OFFEN |
