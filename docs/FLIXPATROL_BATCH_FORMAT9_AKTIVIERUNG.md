...
Stand: 11. September 2026. Aktivierung tatsächlich versucht, aber am ersten realen Titelbündel gescheitert. Die neue Netflix-/Batchkonfiguration ist vollständig zurückgenommen; der erhaltene Format-8-Feed mit öffentlicher Netflix-Wochenquelle bleibt aktiv. Der folgende Vertrag gilt weiter für eine spätere korrigierte Aktivierung.

## Harte Aktivierungsfolge

1. Den kompatiblen Function-/App-Code und beide additiven Migrationen `20260911120000_title_facts_lookup.sql` und `20260911123000_entdecken_flixpatrol_batch_format9.sql` mit allen drei neuen Flags ausgeschaltet ausliefern. Die Foundationmigration führt die zentralen Lookup-/Vocabulary-Verträge und gezählten Requestkinds ein. Zu diesem Stand müssen alle aktiven ausgelieferten Consumer Format 8 und 9 lesen können; der aktive Feed bleibt Format 8.
...
5. Den ersten Lauf beobachten. Max beauftragte ausdrücklich die sofortige Aktivierung: genau ein bestehender Owner-Lauf mit danach zurückgesetztem Owner-Override; kein Workflow-Dispatch, Claimreset oder Retry. Ein späterer neuer Versuch braucht eine konkret geklärte Ursache und eine erneute begrenzte Entscheidung. Format 9 umfasst sieben Tagescharts und höchstens vier Titelbatches; Genres und Keywords dürfen je Lauf jeweils höchstens einen zusätzlichen Request mit höchstens zehn IDs verursachen. Bei einem möglicherweise bezahlten Fehler gibt es keinen Einzel-ID-Fallback und keinen Fullloop-Retry.
...
## Tatsächlicher Nachweis und Blocker

Staging `ee8ef91`, Backend `22c4edf`, beide Migrationen angewandt. Production
`3b82a73` hat seinen Feed ausgeschaltet. Frische Quota- und feste Zwei-ID-
Probe waren erfolgreich. Der anschließende erste Feedlauf hatte zwei gültige
Chartrequests und ein verworfenes Titelbündel (`invalid_response`, HTTP 200).
Drei Versuche vollständig gezählt; kein neuer Feed. Die genaue Mengen-/Typ-/
Feldursache bleibt unbewiesen und wird später untersucht.

Die drei Aktivierungsflags sind entfernt und ihre Abwesenheit bestätigt;
Owner-Override ist aus. Der letzte gute Format-8-Feed blieb bytegleich erhalten,
der letzte Refreshfehler `source_error` als Befund bestehen. Der normale
angemeldete Frontend-Read liefert 50 gültige Titel bei null Anbieterrequests;
der neue Cache-Lookup liefert zwei bekannte frische Fakten. Der Monatszähler
steht auf 53 abgeschlossenen Versuchen. Keine KI-/Watchmode-Anfrage, kein Retry.
Belege: `/private/tmp/kd-data-plan-delivery-20260911/`.
