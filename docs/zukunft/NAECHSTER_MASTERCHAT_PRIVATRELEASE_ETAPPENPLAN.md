...
| M4 | Nur tatsächlich aktive automatische KI-Jobs erhalten genau eine Nachprüfung nach sechs Stunden und höchstens einen Retry | E3b VORBEREITET; 403-STARTGATE OFFEN | Der E3a-Transport ist jetzt gebunden. Der vorhandene automatische Workflow bleibt aber am dokumentierten HTTP-403-Pfad stehen; damit ist weiterhin kein aktiver automatischer Anbieterjob belegt. E3b beginnt zwingend mit der Scheduler-Authentifizierung und genau einem beobachteten planmäßigen Initiallauf, noch ohne Retry. Ledger und Betriebs-Mail können danach disjunkt vorbereitet werden. Die Zuordnung des späteren Radar-Retrys zum exakt selben Ziel benötigt eine schmale additive Scheduler-Naht; ohne ausdrückliche Freigabe dieser technischen Radar-Grenze wird sie nicht gebaut |
...
|---|---|---|---|
| F / Scheduler-Authentifizierung | GitHub-Schedulersecret und Function-Default-Key ohne Wertausgabe fingerprinten, auf denselben modernen Serverkey ausrichten und genau einen planmäßigen Initiallauf beobachten | ausschließlich Workflow-/Secret-Bindung und nötiger Betriebsbeleg; keine Produktlogik | STARTGATE; letzter belegter Radar-/Entdecken-Lauf HTTP 403, daher noch null aktive automatische Anbieterjobs |
| G / Dauerhafter +6h-Zustand | additive service-role-only/RLS-Migration für logische Job-ID, Initialoperation, unveränderliches `check_due_at`, atomaren Due-Claim, `retry_consumed`, Retryoperation und Mailstatus; keine Titel, Suchtexte, Adressen oder Payloads | neue Migration und eigener disposable-Postgres-Test | nach F-Schema-Freeze unabhängig von H/J baubar |
| H / Einmal-Checker | neue kleine Function plus periodischer Workflow; nur fällige allowlistete aktive automatische Anbieterjobs, kein Body, kein Curl-Retry, keine sensitiven Logs | neue `automatic-ai-check`-Function, eigener Workflow und Tests | nach G; providerfreies Entdecken bleibt ausgeschlossen |
| I / Automatischer Radar-Ursprung | Initialclaim atomar mit Scheduler-Ursprung, logischer Job-ID und exaktem Ziel verbinden; Retry mit derselben Job-ID, neuer Provideroperation und allen bestehenden Gates | nur additive RPC/Migration und die kleinste Scheduler-Naht in `radar-websearch-task`; keine Radar-Fach-, UI-, Such- oder Ergebnislogik | VOR BAU AUSDRÜCKLICH FREIZUGEBEN; der heutige bodylose Claim kann sonst nicht sicher dasselbe Ziel wiederholen |
| J / Betriebs-Mail-Bindung | vorhandenen internen `operational-retry`-Builder und Resend-Adapter nach terminalem Retry genau einmal aufrufen; kein Browserweg und kein Mail-Retry | neue serverinterne Checker-Bindung und fokussierte Tests | Inhalt/Transport eingefroren; parallel zu G vorbereitbar, Integration nach H/I |
...
| PR-06 Nachprüfung / P4, E3b | Nur tatsächlich aktive automatische Anbieterfunktionen; initial dauerhaft Job-ID/Funktion/initiale Operation/triggered_at/check_due_at=+6h; atomarer einmaliger fälliger Claim; exakten Initialvorgang prüfen, Reservierung nicht Erfolg; Erfolg nur fertig + finalisierte positive Kosten + gültige Usage/Token; sonst genau ein normaler Retry mit allen Not-Aus-/Rechte-/Provider-/Budget-/Kosten-/Idempotenzgates; gleiche Job-ID/neue Operation/retry_consumed=true; kein neuer Termin/Checkerjob/dritter Versuch; danach eine Betriebs-Mail; keine Jobs für manuelle Aktionen, Mocks, Tests, inaktive Scheduler oder providerfreies Entdecken | Meister bis Dispatch KI-Nachprüfung | VORBEREITET, STARTGATE OFFEN: Der gebundene E3a-Transport ist belegt. Der letzte planmäßige Workflow `33599267721` endete für Radar und Entdecken mit HTTP 403; daher bleibt die Scheduler-Authentifizierung vor jeder Retrylogik zu reparieren und durch genau einen planmäßigen Initiallauf zu belegen. Die disjunkten Pakete F–J stehen oberhalb. Dauerzustand, Checker, technische Radar-Ursprungsbindung und reale Betriebs-Mail-Abnahme bleiben offen; I benötigt vor Code eine ausdrückliche schmale Radar-Freigabe |
